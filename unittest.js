...
describe("Doc File System API Test", () => {
    var dfs, dfs2;
    before((before_done) => {
        dfs = new DocFileSystem(RECONNECT_OPS);
        dfs2 = new DocFileSystem(RECONNECT_OPS);
        dfs.connect(1, () => {
            dfs2.connect(1, () => {
                before_done();
            })
    ...
    it("get file or directory test", () => {
        ...
        assert(dfs.get("/算法/二叉树") === dfs.get("/算法/二叉树/"));
    })
    ...
    it("rename test", () => {
        ...
        assert(objectEqual(dfs.get("/BonjourMonde"), old));
        dfs.mkdir("/Nice/");
        old = dfs.get("/Nice/");
